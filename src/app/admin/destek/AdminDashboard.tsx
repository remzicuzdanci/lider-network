...
import {
  ...
  Zap, ChevronRight, ArrowUpRight, ShieldCheck,
  TrendingUp, Filter,
} from "lucide-react";
...
type Tab = "tickets" | "customers" | "reports";
...
function NavItem({ icon, label, active, badge, badgeColor, onClick }: {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      padding: "9px 12px", borderRadius: "10px", border: "none", width: "100%", textAlign: "left",
...
function Fbtn({ val, cur, set, label }: { val: string; cur: string; set: (v: string) => void; label: string }) {
  ...
  return (
    <button onClick={() => set(val)} style={{
      ...
      border: `1.5px solid ${active ? "#0052ff" : "#e5e7ef"}`,
      background: active ? "#eff6ff" : "transparent",
      color: active ? "#0052ff" : "#6b7280",
...
function StatCard({ label, value, sub, icon, color, bg, border }: {
  ...
  return (
    <div style={{
      background: "#fff", border: `1px solid ${border || "#e5e7ef"}`,
      borderRadius: "14px", padding: "20px 22px", flex: 1, minWidth: 0,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "12px" }}>
        ...
        {!icon && <span style={{ fontSize: "11px", fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: ".5px" }}>{label}</span>}
      </div>
      ...
      <p style={{ fontSize: "12px", color: "#9ca3af", margin: 0 }}>{icon ? label : sub}</p>
      {icon && sub && <p style={{ fontSize: "11px", color: "#9ca3af", margin: "2px 0 0" }}>{sub}</p>}
    </div>
  );
}
...
export default function AdminDashboard() {
  ...
  const rptPriCounts = ["urgent","high","medium","low"].map((p) => ({
    p, label: PRI_LABEL[p], style: PRI_COLOR[p], count: rptTickets.filter((t) => t.priority === p).length,
  }));
  const rptMaxPri = Math.max(...rptPriCounts.map((c) => c.count), 1);

  const rptStatusCounts = ["open","in_progress","resolved","closed"].map((s) => ({
    s, label: STATUS_LABEL[s], style: STATUS_STYLE[s], count: rptTickets.filter((t) => t.status === s).length,
  }));
  ...
  const inpStyle: React.CSSProperties = {
    ...
    border: "1.5px solid #e5e7ef", borderRadius: "8px",
    color: "#1a1d2e", fontSize: "13px", outline: "none", fontFamily: "inherit",
  };
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f4f6fb", fontFamily: "var(--font-family-body)" }}>
      ...
      <aside style={{
        ...
        borderRight: "1px solid #e5e7ef", display: "flex", flexDirection: "column",
        ...
        <div style={{ padding: "20px 18px 16px", borderBottom: "1px solid #f0f2f8" }}>
          ...
          <div style={{ marginTop: "5px", fontSize: "10px", fontWeight: 700, color: "#0052ff", letterSpacing: "1.5px", textTransform: "uppercase" }}>
            Admin Paneli
          </div>
        </div>
        ...
        <div style={{ padding: "12px 14px", borderTop: "1px solid #f0f2f8", display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ margin: 0, fontSize: "12px", fontWeight: 700, color: "#1a1d2e" }}>Admin</p>
            <p style={{ margin: 0, fontSize: "10px", color: "#9ca3af" }}>Lider Network</p>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
          <div>
            <h1 style={{ fontFamily: "var(--font-family-headline)", fontSize: "22px", fontWeight: 800, color: "#1a1d2e", margin: "0 0 3px" }}>
              {tab === "tickets" ? "Destek Talepleri" : tab === "customers" ? "Müşteriler" : "Raporlar"}
            </h1>
            <p style={{ color: "#6b7280", fontSize: "14px", margin: 0 }}>
              {tab === "tickets" ? `${total} talep listeleniyor` : tab === "customers" ? `${customers.length} kayıtlı müşteri` : "Filtreli rapor ve istatistikler"}
            </p>
          </div>
          <button
            onClick={() => { fetchTickets(); fetchStats(); fetchAllTickets(); if (tab === "customers") fetchCustomers(); }}
            ...
            {stats && stats.urgent > 0 && (
              <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "12px", padding: "12px 18px", marginBottom: "16px", display: "flex", alignItems: "center", gap: "10px" }}>
                ...
                <p style={{ margin: 0, fontSize: "13px", color: "#991b1b", fontWeight: 600 }}>
                  {stats.urgent} ACİL öncelikli talep bekliyor — hemen yanıt verilmeli.
                </p>
                <button onClick={() => { setPriorityF("urgent"); }}
                  style={{ marginLeft: "auto", background: "none", border: "1px solid #fca5a5", borderRadius: "6px", padding: "4px 12px", fontSize: "12px", fontWeight: 600, color: "#dc2626", cursor: "pointer" }}>
                  Filtrele
                </button>
              </div>
            )}
            ...
            <div style={{ background: "#fff", borderRadius: "14px", border: "1px solid #e5e7ef", overflow: "hidden" }}>
              {loading ? (
                <div style={{ padding: "56px", textAlign: "center", color: "#9ca3af" }}>
                  <RefreshCw size={28} color="#e5e7ef" style={{ marginBottom: "10px" }} />
                  <p style={{ margin: 0 }}>Yükleniyor...</p>
                </div>
              ) : tickets.length === 0 ? (
                <div style={{ padding: "56px", textAlign: "center", color: "#9ca3af" }}>
                  <TicketCheck size={36} color="#e5e7ef" style={{ marginBottom: "10px" }} />
                  <p style={{ margin: 0 }}>Talep bulunamadı</p>
                </div>
              ) : (
                    ...
                    <thead>
                      <tr style={{ background: "#f8f9fb", borderBottom: "1px solid #e5e7ef" }}>
                        {["#No","Müşteri","Konu","Kategori","Öncelik","Durum","SLA","Tarih",""].map((h) => (
                          <th key={h} style={{ padding: "11px 16px", textAlign: "left", fontSize: "11px", fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: ".5px", whiteSpace: "nowrap" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        return (
                          ...
                          >
                            <td style={{ padding: "14px 16px", whiteSpace: "nowrap" }}>
                              <span style={{ fontSize: "12px", fontWeight: 800, color: "#0052ff", background: "#eff6ff", borderRadius: "6px", padding: "3px 8px" }}>
                            ...
                            <td style={{ padding: "14px 16px", fontSize: "12px", color: "#6b7280", whiteSpace: "nowrap" }}>{CAT_LABEL[t.category]}</td>
                            <td style={{ padding: "14px 16px", whiteSpace: "nowrap" }}>
                            ...
                            <td style={{ padding: "14px 16px", whiteSpace: "nowrap" }}>
                              <span style={{ fontSize: "12px", fontWeight: 600, color: sla.color, background: sla.bg, padding: "3px 8px", borderRadius: "6px" }}>
                                {sla.label}
                              </span>
                            </td>
                            <td style={{ padding: "14px 16px", fontSize: "12px", color: "#9ca3af", whiteSpace: "nowrap" }}>{timeAgo(t.created_at)}</td>
          ...
          <>
            ...
            <div style={{ background: "#fff", borderRadius: "14px", border: "1px solid #e5e7ef", overflow: "hidden" }}>
              {custLoading ? (
                ...
                <div style={{ padding: "56px", textAlign: "center", color: "#9ca3af" }}>
                  <Users size={36} color="#e5e7ef" style={{ marginBottom: "10px" }} />
                  <p style={{ margin: 0 }}>Kayıtlı müşteri bulunamadı</p>
                </div>
              ) : (
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                        ...
                        <td style={{ padding: "14px 16px" }}>
                          {!c.approved ? (
                            <div style={{ display: "flex", gap: "6px" }}>
                              ...
                              <button onClick={() => { if (confirm(`${c.full_name} hesabını reddet ve sil?`)) handleCustomerAction(c.id, "reject"); }} style={{ padding: "5px 12px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "7px", color: "#dc2626", fontSize: "12px", fontWeight: 700, cursor: "pointer" }}>Reddet</button>
                            </div>
                          ) : (
          ...
          <>
            ...
            <div style={{ background: "#fff", border: "1px solid #e5e7ef", borderRadius: "14px", padding: "22px 24px", marginBottom: "18px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "20px", alignItems: "start" }}>
                ...
                {/* Tarih modu */}
                <div>
                  <label style={{ fontSize: "11px", fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: ".5px", display: "block", marginBottom: "8px" }}>Tarih Aralığı</label>
                  <div style={{ display: "flex", gap: "4px", marginBottom: "10px" }}>
                    {[["preset","Hızlı"],["month","Ay"],["range","Aralık"]].map(([m,l]) => (
                      <button key={m} onClick={() => setRptDateMode(m as typeof rptDateMode)}
                        style={{ flex: 1, padding: "6px 0", borderRadius: "8px", border: `1.5px solid ${rptDateMode === m ? "#0052ff" : "#e5e7ef"}`, fontSize: "12px", fontWeight: 600, cursor: "pointer", background: rptDateMode === m ? "#eff6ff" : "#fff", color: rptDateMode === m ? "#0052ff" : "#6b7280" }}>
                        {l}
                      </button>
                    ))}
                  </div>
                    ...
                    <div style={{ display: "flex", gap: "4px", flexWrap: "wrap" }}>
                      {[["today","Bugün"],["week","7 Gün"],["month","30 Gün"],["quarter","90 Gün"],["year","1 Yıl"]].map(([v,l]) => (
                        <button key={v} onClick={() => setRptPreset(v)}
                          style={{ padding: "5px 11px", borderRadius: "8px", border: `1.5px solid ${rptPreset === v ? "#0052ff" : "#e5e7ef"}`, fontSize: "12px", cursor: "pointer", background: rptPreset === v ? "#eff6ff" : "#fff", color: rptPreset === v ? "#0052ff" : "#6b7280" }}>
                          {l}
                        </button>
                      ))}
                    </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "14px", marginBottom: "14px" }}>
              ...
              <div style={{ background: "#fff", border: "1px solid #e5e7ef", borderRadius: "14px", padding: "20px 22px" }}>
                <h4 style={{ margin: "0 0 16px", fontSize: "13px", fontWeight: 700, color: "#1a1d2e", display: "flex", alignItems: "center", gap: "6px" }}>
                  <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#0052ff", display: "inline-block" }} /> Kategoriye Göre
                </h4>
                {rptCatCounts.map((c) => (
              ...
              <div style={{ background: "#fff", border: "1px solid #e5e7ef", borderRadius: "14px", padding: "20px 22px" }}>
                <h4 style={{ margin: "0 0 16px", fontSize: "13px", fontWeight: 700, color: "#1a1d2e", display: "flex", alignItems: "center", gap: "6px" }}>
                  <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#dc2626", display: "inline-block" }} /> Önceliğe Göre
                </h4>
                {rptPriCounts.map((p) => (
                  <div key={p.p} style={{ marginBottom: "12px" }}>
                    ...
                    <div style={{ height: "6px", background: "#f0f2f8", borderRadius: "3px", overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${(p.count / rptMaxPri) * 100}%`, background: p.style.text, borderRadius: "3px", opacity: 0.8, transition: "width .5s" }} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ background: "#fff", border: "1px solid #e5e7ef", borderRadius: "14px", padding: "20px 22px" }}>
                <h4 style={{ margin: "0 0 16px", fontSize: "13px", fontWeight: 700, color: "#1a1d2e", display: "flex", alignItems: "center", gap: "6px" }}>
                  <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#22c55e", display: "inline-block" }} /> Duruma Göre
                </h4>
                {rptStatusCounts.map((s) => (
                  <div key={s.s} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 14px", marginBottom: "8px", background: s.style.bg, borderRadius: "8px", border: `1px solid ${s.style.bg}` }}>
            ...
            {/* Ticket table */}
            {rptTickets.length > 0 && (
              <div style={{ background: "#fff", border: "1px solid #e5e7ef", borderRadius: "14px", overflow: "hidden" }}>
                <div style={{ padding: "14px 20px", borderBottom: "1px solid #f0f2f8", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#f8f9fb" }}>
                  <span style={{ fontSize: "13px", fontWeight: 700, color: "#1a1d2e" }}>Talep Listesi</span>
                  <span style={{ fontSize: "12px", color: "#9ca3af", background: "#e5e7ef", padding: "2px 10px", borderRadius: "10px", fontWeight: 600 }}>{rptTickets.length} talep</span>
                </div>
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ borderBottom: "1px solid #f0f2f8" }}>
                        {["#No","Müşteri","Konu","Kategori","Öncelik","Durum","Tarih"].map((h) => (
                          <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontSize: "11px", fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: ".5px" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {rptTickets.map((t) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={{ padding: "12px 16px", maxWidth: "220px" }}><p style={{ margin: 0, fontSize: "13px", color: "#1a1d2e", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.subject}</p></td>
                            <td style={{ padding: "12px 16px", fontSize: "12px", color: "#6b7280" }}>{CAT_LABEL[t.category]}</td>
                            <td style={{ padding: "12px 16px" }}>
                              <span style={{ fontSize: "12px", fontWeight: 700, color: pc.text, background: pc.bg, padding: "3px 10px", borderRadius: "6px" }}>{PRI_LABEL[t.priority]}</span>
                            </td>
                            <td style={{ padding: "12px 16px" }}>
